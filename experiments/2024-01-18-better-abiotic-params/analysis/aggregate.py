...
def main():
    ...
    # Aggregates output from "world_summary.csv"
    world_summary_content_lines = []
    ...
    world_summary_header = None
    world_summary_fpath = os.path.join(dump_dir, "world_summary.csv")
    ...
    for run_dir in run_dirs:
        ...
        required_files = [
            os.path.join("output", "world_summary.csv"),
        ...
        shared_summary_info["interaction_matrix"] = interaction_matrix
        # "_".join(interaction_matrix.split("_")[:-1])

        print("Run configuration:", shared_summary_info)
        ############################################################

        ############################################################
        # Extract world summary (includes data over time)
        run_data_path = os.path.join(run_path, "output", "world_summary.csv")
        run_data = utils.read_csv(run_data_path)

        world_summary_content_lines = []
        for line in run_data:
            world_summary_content_lines.append(line)
            for field in shared_summary_info:
                world_summary_content_lines[-1][field] = shared_summary_info[field]

        if world_summary_header == None:
            world_summary_header = write_csv(world_summary_fpath, world_summary_content_lines)
        else:
            append_csv(world_summary_fpath, world_summary_content_lines, world_summary_header)
